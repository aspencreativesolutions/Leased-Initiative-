...
function buildPdf() {
  ...
  h3('Official Tenants')
  bullet('Signed leases that are active or starting soon')
  bullet('Mobile: scrollable tiles (two per row by default; switch to one); help icon for section copy')
  bullet('Desktop: spreadsheet with Edit Columns')
  ...
  h3('Display Settings')
  bullet('Mobile: scrollable tiles only (two per row by default; switch to one); help icon for page copy')
  bullet('Desktop: Tile vs Spreadsheet view (persisted)')
  bullet('Location filters: Property State, Town, Group (+ Edit Groups)')
  ...
  bullet('Display Settings: mobile tiles (2/1 per row); desktop Tile (default) / Spreadsheet; Filter')
  bullet('Status filters: Any, Signed, Sent (plus location filters)')
  bullet('Location: Property State, Area Code, Group (+ Edit Groups)')
  bullet('Tiles: tenant name; Sent/Signed badges with hover term dates; address → map; Month X of Y timeline; Open; Delete')
  ...
  bullet('Mobile tiles (2/1 per row) for Rentals, Official Tenants, and Lease Agreements; desktop Tile vs Spreadsheet (Rentals, Lease Agreements); Payments tiles + scale')
  bullet('Occupancy color coding on Rental tiles')
  bullet('Lease status badges (Sent / Signed + hover term dates)')
