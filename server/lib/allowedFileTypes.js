...
/** Repair / concern submissions require a photo (not PDF/Word). */
export function isAllowedPhotoUpload(file) {
  const ext = path.extname(file.originalname || '').toLowerCase()
  if (ALLOWED_PHOTO_EXTENSIONS.has(ext)) return true
  if (file.mimetype && ALLOWED_PHOTO_MIMES.has(file.mimetype)) return true
  return false
}
